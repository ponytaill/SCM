...
const genInputCode = (option, runtime) => {
    if (runtime) {
      ...
      commonScriptElem.addEventListener('load', function() {
        window.mobileCommonFuncs.getDimPath().then(
          function(dimPath) {
            System.import("./${option.publicPath}" + dimPath + "${option.fileName}?v=${option.version}");
          },
          function() {
            alert('应用加载失败，请联系管理员');
          }
    ...
    `;
        ...
        return `System.import("./${option.publicPath}${option.fileName}?v=${option.version}");`;
    }
};
exports.default = (options) => {
    ...
    return ({ attributes, files, meta, publicPath, title }) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const scripts = (files.js || []).filter((chunk) => chunk.isEntry)
            ...
            return `<script ${attrs}>${genInputCode(Object.assign(Object.assign({}, chunk), { publicPath, version }), !!isRtc)}</script>`;
        })
            .join('\n');
  ...
  <head>
    ...
    ${links}
    <script src="https://ccwork.oss.cn-north-3.inspurcloudoss.com/h5/ccworkJsbridge.js"></script>
